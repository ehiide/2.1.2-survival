...
public class ChatUtil 
{
	...
	public static String addDruggedEmotes(final Player player, String message)
	{
		...
		if (serotonine < 0)
		{
			...
			if (MathUtil.chanceOf(Math.abs(serotonine)))
		...
		if (serotonine > 0)
		{
			...
			if (MathUtil.chanceOf(serotonine))
				message = messages.get(new Random().nextInt(messages.size()));
			...
			if (MathUtil.chanceOf(serotonine))
				message = message + " " + ustawLosowaEmotke("smile");
		}
			...
			public void run()
			{
				...
				if (MathUtil.chanceOf(50))
				{
					if (MathUtil.chanceOf(10))
						ChatManager.sendMessage(player, ColorUtil.formatHEX("#80ff1f[" + TimeUtil.hour() + ":" + TimeUtil.minute() + "#80ff1f] " + ChatUtil.getGangInChat(DataManager.getInstance().getLocal(onlinePlayer).getGang()) + ChatUtil.returnMarryPrefix(onlinePlayer) + "&r" + ChatUtil.returnPlayerColor(onlinePlayer) + Objects.requireNonNull(onlinePlayer.getPlayer()).getName() + "#8c8c8c " +
	...
	public static String getPlaceholder(final Player player, String placeholder)
	{
		...
		placeholder = placeholder.replace("%questy%", "#8c8c8c" + QuestUtil.getCompletedQuests(player) + " ukonczonych questow" + " #666666&o(%questy%)#8c8c8c");
	...
	public static String centerString(@NotNull String string, final int hexes, final int bukkitcodes)
	{
